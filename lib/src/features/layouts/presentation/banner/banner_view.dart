...
class BannerView extends ConsumerWidget {
  ...
  void _onBannerTap(BuildContext context, Banner banner, WidgetRef ref) {
    ...
    if (banner.bannerType == 'Item Group' && banner.bannerLink != null) {
      context.pushRoute(ItemGroupScreen(itemGroupId: banner.bannerLink!));
    } else if (banner.bannerType == 'Website Item') {
      ...
      if (banner.udhiyah == 1) {
        ...
        context
            .pushRoute(MubadaraItemsScreen(mubadaraId: banner.bannerLink!))
            .then((index) {
          if (index != null) {

...
final isExpress=deliveryTypes.any( (d)=>d.deliveryMethodTitle.toLowerCase()
  ...
  Widget build(BuildContext context) {
    ...
    return cartAsync.when(
              ...
              child: SingleChildScrollView(
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      if (cart.pickup != 1) _buildAddressSection(cart),
                      const SizedBox(height: 20),
                      _buildPaymentSection(cart),
                      const SizedBox(height: 20),
                      if (cart.mubadara != 1) _buildCouponSection(cart),
                      const SizedBox(height: 20),
                      Text(S.of(context).orderSummary, style: _textStyle()),
                      const SizedBox(height: 8),
                      OrderSummaryContainer(cart: cart),
                      const SizedBox(height: 20),
                      Text(S.of(context).reviewYourOrder, style: _textStyle()),
      ...
      error: (e, st) => AppBanner(message: e.toString(), stackTrace: st),
      loading: () => const FadeCircleLoadingIndicator(),
    );
  }

  Widget _buildAddressSection(Cart cart) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
          ...
          children: [
            Text(S.of(context).shipTo, style: _textStyle()),
...
class _PaymentMethodCard extends StatelessWidget {
  const _PaymentMethodCard({required this.cart});
  final Cart cart;
